...
///start
   ...
   {
             //use a parameter PageTitle {{.PageTitle}} set in net_v1.go with the server url
        $.getJSON("{{.PageTitle}}/v1/getDash02Grafica02?reference="+previousGroupNumber0104+"&reference2=223&dato01="+paramDataType +"&dato02=88&dato03=V2", function(data) {
            var linea=0;
...
//only one		updateHOYChartWith("01");
//only one		updateHOYChartWith("02");

		updateChartWith7dias("70");
//only one		updateChartWith7dias("71");
//only one		updateChartWith7dias("72");
		
		updateChartWith30dias("30");
//only one		updateChartWith30dias("31");
//only one		updateChartWith30dias("32");
...
//            setInterval(updateChart0201, valueRotacion);//every 20 seconds
    ...
    function go_next2() {
//        alert("next");
        window.location.href = "/v2/dash07";  
