$(function () {
	...
	function populatePageViewsCharts() {
        ...
        console.log("json banwire populatePageViewsCharts "); 
...
//        $.getJSON("https://canvasjs.com/services/data/datapoints.php?xstart=1&ystart=10&length=100&type=json", function(data) {  
        $.getJSON("https://8090-dot-3809294-dot-devshell.appspot.com/v1/getjsondatabanwire", function(data) {  
  ...
  //              pageViewsPerSecondDataPoints.push({ x: time1, y: initialDataPageViewsPerSecond[i] });
                pageViewsPerMinuteDataPoints.push({ x: time2,y: parseInt(value[1]) });
            
                 console.log("json banwire"+value[1]); 
    ...
    sumYValuePageViewsPerSecond += yValuePageViewsPerSecond;
		...
		if (numberOfSeconds >= 60) {
			time2 = new Date();
			time2.setSeconds(0);
			console.log("json banwire recent GET");
